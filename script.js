document.addEventListener('DOMContentLoaded', () => {
    ...
    // ----------------------------------------------------------------------
// 17. Logika Kalkulator Harga Pokok Penjualan (HPP)
// ----------------------------------------------------------------------
if (getElement('calculateHppBtn')) {
    const persediaanAwalInput = getElement('persediaanAwal');
    const pembelianBersihInput = getElement('pembelianBersih');
    const persediaanAkhirInput = getElement('persediaanAkhir');
    const calculateHppBtn = getElement('calculateHppBtn');
    const barangTersediaEl = getElement('barangTersedia');
    const hppResultEl = getElement('hppResult');
    const hppError = getElement('hppError');

    // --- Format Rupiah ---
    // (Asumsikan fungsi formatIDR sudah didefinisikan di atas, misal dari Budget Tracker)
    ...
    const calculateHPP = () => {
        hppError.classList.add('hidden');
        
        // Ambil nilai dan pastikan nilainya adalah angka
        const awal = parseInt(persediaanAwalInput.value) || 0;
        const beli = parseInt(pembelianBersihInput.value) || 0;
        const akhir = parseInt(persediaanAkhirInput.value) || 0;

        if (awal < 0 || beli < 0 || akhir < 0) {
            hppError.textContent = 'Semua nilai input harus nol atau positif.';
            hppError.classList.remove('hidden');
            return;
        }

        // 1. Hitung Barang Tersedia untuk Dijual
        const barangTersedia = awal + beli;
        
        // 2. Hitung HPP
        const hpp = barangTersedia - akhir;

        // Tampilkan Hasil
        barangTersediaEl.textContent = formatIDR(barangTersedia);
        hppResultEl.textContent = formatIDR(hpp);
    };

    // --- Event Listeners ---
    calculateHppBtn.addEventListener('click', calculateHPP);
    
    // Opsional: Hitung ulang secara real-time saat input berubah
    persediaanAwalInput.addEventListener('input', calculateHPP);
    pembelianBersihInput.addEventListener('input', calculateHPP);
    persediaanAkhirInput.addEventListener('input', calculateHPP);

    // --- Inisialisasi ---
    calculateHPP(); // Hitung nilai awal (semua 0)
}

// ----------------------------------------------------------------------
// 20. Logika Penghitung Karakter dan Kata
// ----------------------------------------------------------------------
if (getElement('textInput')) {
    const textInput = getElement('textInput');
    const charCountEl = getElement('charCount');
    const wordCountEl = getElement('wordCount');

    // --- Fungsi Utama Penghitung ---
    const updateCount = () => {
        const text = textInput.value;
        
        // 1. Hitung Karakter: panjang string mentah (termasuk spasi dan baris baru)
        const charCount = text.length;

        // 2. Hitung Kata:
        // Pisahkan kata berdasarkan spasi. Hapus spasi di awal/akhir, lalu ganti
        // beberapa spasi berturut-turut dengan satu spasi, lalu pisahkan.
        const cleanedText = text.trim();
        let wordCount = 0;
        
        if (cleanedText.length > 0) {
            // Memisahkan string berdasarkan spasi dan/atau baris baru (\s+)
            // Filter(Boolean) untuk menghilangkan elemen kosong yang mungkin dihasilkan dari spasi berlebih
    ...
    // --- Inisialisasi ---
    updateCount(); // Hitung nilai awal (0)
}
});
